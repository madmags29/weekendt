...
export default function LandingPage({ onSearch }: LandingPageProps) {
    ...
    useEffect(() => {
        ...
        fetch(`${API_URL}/background-videos`)
            ...
            .catch(err => console.error("Failed to fetch background video", err));
        ...
        <div className="relative h-screen w-screen overflow-hidden bg-black text-white font-sans">
            ...
            <div className={`absolute inset-0 transition-opacity duration-1000 ${loaded ? 'opacity-100' : 'opacity-0'}`}>
                ...
                {videoData && videoData.credit && (
                    <div className="absolute bottom-4 right-4 z-20 text-[10px] text-zinc-500/50 uppercase tracking-widest pointer-events-none">
            ...
            {/* Sidebar Panel */}
            <div className={`absolute top-0 left-0 bottom-0 z-50 w-72 bg-zinc-900/90 backdrop-blur-xl border-r border-white/10 shadow-2xl transition-transform duration-300 ease-in-out transform ${showMenu ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-6 flex flex-col h-full">
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-xl font-bold tracking-tight">Menu</h2>
                        <button onClick={() => setShowMenu(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                            <X className="w-5 h-5" />
                        </button>
                    </div>
            ...
            <button
                ...
                className={`absolute top-6 left-6 z-30 p-2.5 bg-black/20 hover:bg-black/40 backdrop-blur-md border border-white/10 rounded-full text-white transition-all hover:scale-105 ${showMenu ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
            >
            ...
            <div className="relative z-10 flex flex-col items-center justify-center h-full px-4 text-center">
                ...
                <form onSubmit={handleSearch} className="w-full max-w-2xl mb-16 group relative">
                    ...
                    <div className="relative flex items-center bg-white/10 backdrop-blur-xl border border-white/20 rounded-full p-2 transition-all group-focus-within:bg-black/40 group-focus-within:border-white/40 shadow-2xl">
                        ...
                        <input
                            ...
                            autoComplete="off"
                            className="flex-1 bg-transparent border-none text-white text-lg px-4 py-3 focus:ring-0 focus:outline-none placeholder-zinc-400 font-light"
                        />
                        <button
                            type="submit"
                ...
                {loadingRecs ? (
                    <div className="text-zinc-400 animate-pulse">Finding hidden gems near you...</div>
                ) : (
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 w-full max-w-5xl px-4">
                        {destinations.map((dest) => (
                            <div
                                ...
                                className="group relative h-48 md:h-64 rounded-2xl overflow-hidden cursor-pointer border border-white/10 hover:border-white/40 transition-all shadow-lg hover:-translate-y-1"
                            >
                                <img
                                    src={dest.image}
                                    alt={dest.name}
